  ...
  return (
    <div style={{ background: "#fff", borderRadius: 20, padding: "20px", border: "1px solid #f1f5f9", marginBottom: 14 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        ...
        <a href={"https://wa.me/?text=" + encodeURIComponent((listing?.title || "Check out this property") + " on FindWithHabi: " + (typeof window !== "undefined" ? window.location.href : ""))}
          target="_blank" rel="noopener noreferrer"
          style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px", borderRadius: 11, background: "#25D366", color: "#fff", textDecoration: "none", fontSize: 13, fontWeight: 700 }}>
        ...
        <a href={"https://twitter.com/intent/tweet?text=" + encodeURIComponent((listing?.title || "Check out this property") + " on FindWithHabi") + "&url=" + encodeURIComponent(typeof window !== "undefined" ? window.location.href : "https://findwithhabi.com")}
          target="_blank" rel="noopener noreferrer"
          style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "10px 12px", borderRadius: 11, background: "#000", color: "#fff", textDecoration: "none", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
        ...
        <button onClick={() => {
          navigator.clipboard?.writeText(window.location.href)
            .then(() => showToast("🔗 Link copied!", "copy"))
            .catch(() => showToast("Could not copy link", "error"))
        }}
          style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 14px", borderRadius: 11, border: "1.5px solid #e2e8f0", background: "#f8fafc", color: "#64748b", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
          <Copy size={14} />
        </button>
      </div>
    </div>
...
export default function ListingDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchListing = async () => {
      ...
      try {
        const key = "fwh_recently_viewed"
        const existing = JSON.parse(localStorage.getItem(key) || "[]")
        const filtered = existing.filter(l => l.id !== listingData.id)
        const updated = [{ id: listingData.id, title: listingData.title, price_label: listingData.price_label, cover_image: null }, ...filtered].slice(0, 10)
      ...
      setAgent(agentData)
      // Update recently viewed with cover image
      try {
        const cover = imagesData?.find(i => i.is_cover)?.url || imagesData?.[0]?.url || null
        if (cover) {
          const key = "fwh_recently_viewed"
          const existing = JSON.parse(localStorage.getItem(key) || "[]")
          const updated = existing.map(l => l.id === id ? { ...l, cover_image: cover } : l)
          localStorage.setItem(key, JSON.stringify(updated))
        }
      } catch(e) {}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "72px 12px calc(100px + env(safe-area-inset-bottom, 0px))" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 20 }} className="detail-layout">
          ...
          <div style={{ minWidth: 0 }}>
            ...
            {/* Nearby Amenities */}
            {listing?.city && (
              <div style={{ background: "#fff", borderRadius: 20, padding: "24px", border: "1px solid #f1f5f9", marginBottom: 16 }}>
                <h3 style={{ margin: "0 0 16px", fontSize: 16, fontWeight: 900, color: "#0d1f2d", display: "flex", alignItems: "center", gap: 8 }}>
                  📍 What's Nearby
                </h3>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px,1fr))", gap: 10 }}>
                  {[
                    { icon: "🏫", label: "Schools", desc: "Within 2km" },
                    { icon: "🏥", label: "Hospitals", desc: "Within 3km" },
                    { icon: "🛒", label: "Markets", desc: "Walking distance" },
                    { icon: "🚌", label: "Transport", desc: "Bus stops nearby" },
                    { icon: "🏦", label: "Banks & ATMs", desc: "Within 1km" },
                    { icon: "⛽", label: "Fuel Stations", desc: "Nearby" },
                  ].map((item, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px", borderRadius: 12, background: "#f8fafc", border: "1px solid #f1f5f9" }}>
                      <span style={{ fontSize: 20, flexShrink: 0 }}>{item.icon}</span>
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 800, color: "#0d1f2d" }}>{item.label}</div>
                ...
                <p style={{ margin: "12px 0 0", fontSize: 11, color: "#94a3b8" }}>
                  * Amenity estimates based on {listing.city}, {listing.state}. Verify exact distances before committing.
                </p>
              </div>
            )}
